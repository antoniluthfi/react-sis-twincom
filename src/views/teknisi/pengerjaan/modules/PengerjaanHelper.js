...
const PengerjaanHelper = () => {
    ...
    const submitHandler = action => {
        if(action === 'Submit') {
            ...
            deleteDataPengerjaan(currentPengerjaan.no_pengerjaan);
        }
    }
    ...
    const getDataPengerjaan = async id_teknisi => {
        await axios.get(`${baseUrl}/pengerjaan/barang-jasa/teknisi/${id_teknisi}`, {
            headers: {
                'Accept': 'Application/json',
                'Authorization': `Bearer ${localStorage.getItem('sis-token')}`
            }
    ...
    const postDataPengerjaan = async no_pengerjaan => {
        let inputBiaya;
        if(input.biaya_service.indexOf('.') > 0 || input.biaya_service.indexOf(',') > 0) {
        ...
        await axios.put(`${baseUrl}/pengerjaan/${no_pengerjaan}`, {
            id_partner: input.id_partner == null ? currentPengerjaan.id_partner : input.id_partner,
    ...
    const updateDataPengerjaan = async no_pengerjaan => {
        ...
        if(input.diskon_persen != '') {
            var biaya_service = inputBiaya - (inputBiaya / 100 * input.diskon_persen);
        } else if(input.diskon_langsung != '') {
            var biaya_service = inputBiaya - input.diskon_langsung;
        } else {
            var biaya_service = inputBiaya;
        }
    ...
    const updateDetailPengerjaan = async no_pengerjaan => {
        ...
        .then(response => {
            if((input.status_pengerjaan == '3' || input.status_pengerjaan == '1') && (currentPengerjaan.penerimaan.jenis_penerimaan != 'Jasa Lain-lain')) {
                getDataPengembalianByNoService(currentPengerjaan.no_service);
            }

            if(input.status_pengerjaan == '3' && currentPengerjaan.penerimaan.jenis_penerimaan == 'Jasa Lain-lain') {
                getDataPembayaranByNoService(currentPengerjaan.no_service);
            }
        })
        .catch(error => {
            Swal.fire(
    ...
    const getDataPengembalianByNoService = async no_service => {
    ...
    const getDataPembayaranByNoService = async no_service => {
        await axios.get(`${baseUrl}/pembayaran/no-service/${no_service}`, {
            headers: {
                'Accept': 'Application/json',
                'Authorization': `Bearer ${localStorage.getItem('sis-token')}`
            }
    ...
    const deleteDataPengerjaan = async no_pengerjaan => {
        ...
        .then(response => {
            ...
            writeActivityLog(currentUser.id, currentUser.jabatan, 'Pengerjaan Barang & Jasa', 'UPDATE', `${currentUser.name} mereset data nomor pengerjaan ${currentPengerjaan.no_pengerjaan}`);
        })
        .catch(error => {
            Swal.fire(
                'Gagal',
    ...
    const sendEmailNotification = async payload => {
        await axios.post(`${baseUrl}/pengerjaan/send-email`, {
            ...
            name: payload.penerimaan.customer.name,
