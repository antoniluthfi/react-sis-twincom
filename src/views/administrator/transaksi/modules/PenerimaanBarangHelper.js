...
const PenerimaanBarangHelper = () => {
    ...
    const getDataPenerimaanBarang = async () => {
        await axios.get(`${baseUrl}/penerimaan-barang`, {
            headers: {
                'Accept': 'Application/json',
                'Authorization': `Bearer ${localStorage.getItem('sis-token')}`
            }
    ...
    const changeHandler = event => {
        ...
        if(event.target.name === 'merek') getDataTipeByMerek(currentBj.nama_bj, event.target.value);
    ...
    const postDataPenerimaanBarang = async () => {
        ...
        else if(!input.id_cabang) message = 'Cabang harus dipilih salah satu!';
    ...
    const postListKelengkapan = async (no_service, action) => {
    ...
    const postListRating = async no_service => {
        const teknisi_name = input.id_teknisi; // id berisi nama, bukan id
        await axios.post(`${baseUrl}/review-kepuasan-pelanggan`, {
            no_service: no_service,
            user_id: `${teknisi_name},${currentAdmin.name}`,
    ...
    const getDataTipeByMerek = async (kategori, merek) => {
        await axios.get(`${baseUrl}/tipe/kategori/${kategori}/merek/${merek}`, {
            headers: {
                'Accept': 'Application/json',
                'Authorization': `Bearer ${localStorage.getItem('sis-token')}`                   
            }
    ...
    const postDataTipe = async () => {
        ...
        if(message != null) {
            ...
            .then(response => {
                ...
                getDataTipeByMerek(currentBj.nama_bj, inputTipe.merek);
