...
const DashboardReviewerHelper = () => {
    ...
    const getCurrentUser = async () => {
        ...
        .then(response => {
            ...
            getDataPenerimaan(response.data.data.cabang.id);
        })
        .catch(error => {
            console.log(error);
        });
    ...
    const getDataPenerimaan = async id_cabang => {
        await axios.get(`${baseUrl}/penerimaan-barang/cabang/${id_cabang}`, {
            headers: {
                'Accept': 'Application/json',
                'Authorization': `Bearer ${localStorage.getItem('sis-token')}`
            }
        ...
        .then(response => {
            ...
            if(response.data.data[0].arus_kas != null) {
                setSuccess(true);
                setCurrentPenerimaan(response.data.data[0]);
            }
        })
        .catch(error => {
            console.log(error);
    ...
    const postReviewKepuasan = async () => {
...
export default DashboardReviewerHelper;