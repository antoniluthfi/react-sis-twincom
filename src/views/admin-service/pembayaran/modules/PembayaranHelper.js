...
const PembayaranHelper = () => {
    ...
    const getCurrentUser = async () => {
        ...
        .then(response => {
            getDataPembayaran(response.data.data.cab_penempatan);
            setCurrentUser(response.data.data);
            getDataAdmin(response.data.data.cab_penempatan);
        })
        .catch(error => {
            console.log(error);
        });
    ...
    const getDataPembayaran = async cabang => {
        await axios.get(`${baseUrl}/pembayaran/cabang/${cabang}`, {
            headers: {
                'Accept': 'Application/json',
                'Authorization': `Bearer ${localStorage.getItem('sis-token')}`
            }
    ...
    const updateDataPembayaran = async no_pembayaran => {
        ...
        if((parseInt(currentPembayaran.pengerjaan.biaya_service) == parseInt(inputBiaya)) || (parseInt(currentPembayaran.pengerjaan.biaya_service) < parseInt(inputBiaya))) {
            status_pembayaran = 1;
        }
