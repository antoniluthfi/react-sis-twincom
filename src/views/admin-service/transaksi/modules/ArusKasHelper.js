...
const ArusKasHelper = () => {
    ...
    const submitHandler = action => {
        if(action === 'Submit') {
            ...
            deleteDataArusKas(currentArusKas.no_bukti);
    ...
    const getCurrentUser = async () => {
        ...
        .then(response => {
            getDataArusKas(response.data.data.id);
    ...
    const getDataArusKas = async id_admin => {
        await axios.get(`${baseUrl}/arus-kas/admin/${id_admin}`, {
            headers: {
                'Accept': 'Application/json',
                'Authorization': `Bearer ${localStorage.getItem('sis-token')}`
            }
